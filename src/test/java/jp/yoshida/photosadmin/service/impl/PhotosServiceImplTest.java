package jp.yoshida.photosadmin.service.impl;
...
import jp.yoshida.photosadmin.common.constant.StandardsConstants;
...
import jp.yoshida.photosadmin.common.util.PhotosUtil;
import jp.yoshida.photosadmin.repository.PhotosRepository;
...
class PhotosServiceImplTest {
    ...
    void setUp() {
        ...
        photosService = new PhotosServiceImpl(photosRepository, validator);
    ...
    void getPhotosN1() {
        ...
        doReturn(photos).when(photosRepository).getPhotos();
    ...
    void getPhotoN1() throws PhotosBusinessException {
        ...
        doReturn(ids).when(photosRepository).getPhotoIds();
        doReturn(photo1).when(photosRepository).getPhoto(id1);
    ...
    void getPhotoE1() {
        ...
        int id = 0;
        doReturn(ids).when(photosRepository).getPhotoIds();
        doReturn(null).when(photosRepository).getPhoto(id);
    ...
    void addPhotoN1() throws IOException, PhotosSystemException, PhotosBusinessException {
        ...
        try (MockedStatic<PhotosUtil> photosUtil = mockStatic(PhotosUtil.class)) {
            ...
            doNothing().when(photosRepository).addPhoto(photo);
    ...
    void addPhotoE2() throws IOException {
        ...
        try (MockedStatic<PhotosUtil> photosUtil = mockStatic(PhotosUtil.class)) {
            photosUtil.when(() -> PhotosUtil.createThumbnail(sendingPhoto)).thenReturn(thumbnail1);
            photosUtil.when(() -> PhotosUtil.setMetaDatum(photo, sendingPhoto)).thenAnswer((invocation) -> invocation);
    ...
    void deletePhotoN1() throws IOException {
        ...
        int[] ids = {101, 102, 103};
        doReturn(3).when(photosRepository).deletePhotos(ids);
    ...
    void deletePhotoE1() throws IOException {
        ...
        int[] ids = {101, 102, 103};
        doReturn(0).when(photosRepository).deletePhotos(ids);
        PhotosBusinessException e
